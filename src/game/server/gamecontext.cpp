...
void CGameContext::Construct(int Resetting)
{
	...
	if(Resetting == NO_RESET)
	{
		...
		for(int i = 0; i < NUM_DDRACE_TEAMS; i++)
		{
			...
			m_aSavedGomokuGames[i] = new SGomokuSavedGame();
			m_aSavedConnect4Games[i] = new SConnect4SavedGame();
			m_aSavedFlightChessGames[i] = new SFlightChessSavedGame();
		}
	}
	...
	std::fill(std::begin(m_aDuelRequester), std::end(m_aDuelRequester), -1);
	mem_zero(m_aDuelRequestTick, sizeof(m_aDuelRequestTick));
	mem_zero(m_aDuelRequestWeapons, sizeof(m_aDuelRequestWeapons));
	mem_zero(m_aDuelTuningRestoreTick, sizeof(m_aDuelTuningRestoreTick));
	std::fill(std::begin(m_aGomokuRequester), std::end(m_aGomokuRequester), -1);
	mem_zero(m_aGomokuRequestTick, sizeof(m_aGomokuRequestTick));
	std::fill(std::begin(m_aConnect4Requester), std::end(m_aConnect4Requester), -1);
	mem_zero(m_aConnect4RequestTick, sizeof(m_aConnect4RequestTick));
	for(SGameInviteVote &InviteVote : m_aGameInviteVotes)
...
void CGameContext::Destruct(int Resetting)
{
	...
	if(Resetting == NO_RESET)
	{
		...
		for(int i = 0; i < NUM_DDRACE_TEAMS; i++)
		{
			...
			delete m_aSavedGomokuGames[i];
			m_aSavedGomokuGames[i] = nullptr;
			delete m_aSavedConnect4Games[i];
			m_aSavedConnect4Games[i] = nullptr;
			delete m_aSavedFlightChessGames[i];
			m_aSavedFlightChessGames[i] = nullptr;
		}
	}
...
void CGameContext::Clear()
{
	...
	m_QQMessageCooldowns = QQMessageCooldowns;
...
void CGameContext::CreateExplosion(vec2 Pos, int Owner, int Weapon, bool NoDamage, int ActivatedTeam, CClientMask Mask)
{
	...
	for(int i = 0; i < Num; i++)
	{
		...
		float Strength;
		if(Owner != -1 && IsDuelParticipant(Owner))
			Strength = CTuningParams::DEFAULT.m_ExplosionStrength;
		else if(Owner == -1 || !m_apPlayers[Owner] || !m_apPlayers[Owner]->m_TuneZone)
...
{
	dbg_assert(in_range(ToClientId, 0, MAX_CLIENTS - 1), "SendModeratorAlert ToClientId invalid: %d", ToClientId);
	...
	else
	{
		...
		log_info("moderator_alert", "Player uses an old client; sent broadcast fallback to %s (ID %d)", Server()->ClientName(ToClientId), ToClientId);
	}
}
...
void CGameContext::SendVoteSet(int ClientId)
{
	...
	if(ClientId == -1)
	{
		for(int i = 0; i < Server()->MaxClients(); i++)
		{
			if(!m_apPlayers[i])
				continue;
			if(HasGameInviteVote(i))
			{
				SendGameInviteVote(i);
				continue;
			}
			if(!Server()->IsSixup(i))
	...
	{
		if(HasGameInviteVote(ClientId))
		{
			SendGameInviteVote(ClientId);
			return;
		}
...
void CGameContext::SendTuningParams(int ClientId, int Zone)
{
	...
	// A duelist keeps the map's movement tuning, but receives the original DDNet tuning
	// for every weapon-related parameter. This is also what the local client
	// predicts, so weapon feel no longer falls back to the current tune zone.
	if(IsDuelParticipant(ClientId))
	{
		const CTuningParams &DuelDefaults = CTuningParams::DEFAULT;
		DuelTuning = Zone == 0 ? m_Tuning : m_aTuningList[Zone];
		DuelTuning.m_GunCurvature = DuelDefaults.m_GunCurvature;
		DuelTuning.m_GunSpeed = DuelDefaults.m_GunSpeed;
		DuelTuning.m_GunLifetime = DuelDefaults.m_GunLifetime;
		DuelTuning.m_ShotgunCurvature = DuelDefaults.m_ShotgunCurvature;
		DuelTuning.m_ShotgunSpeed = DuelDefaults.m_ShotgunSpeed;
		DuelTuning.m_ShotgunSpeeddiff = DuelDefaults.m_ShotgunSpeeddiff;
		DuelTuning.m_ShotgunLifetime = DuelDefaults.m_ShotgunLifetime;
		DuelTuning.m_GrenadeCurvature = DuelDefaults.m_GrenadeCurvature;
		DuelTuning.m_GrenadeSpeed = DuelDefaults.m_GrenadeSpeed;
		DuelTuning.m_GrenadeLifetime = DuelDefaults.m_GrenadeLifetime;
		DuelTuning.m_LaserReach = DuelDefaults.m_LaserReach;
		DuelTuning.m_LaserBounceDelay = DuelDefaults.m_LaserBounceDelay;
		DuelTuning.m_LaserBounceNum = DuelDefaults.m_LaserBounceNum;
		DuelTuning.m_LaserBounceCost = DuelDefaults.m_LaserBounceCost;
		DuelTuning.m_LaserDamage = DuelDefaults.m_LaserDamage;
		DuelTuning.m_ShotgunStrength = DuelDefaults.m_ShotgunStrength;
		DuelTuning.m_ExplosionStrength = DuelDefaults.m_ExplosionStrength;
		DuelTuning.m_HammerStrength = DuelDefaults.m_HammerStrength;
		DuelTuning.m_HammerFireDelay = DuelDefaults.m_HammerFireDelay;
		DuelTuning.m_GunFireDelay = DuelDefaults.m_GunFireDelay;
		DuelTuning.m_ShotgunFireDelay = DuelDefaults.m_ShotgunFireDelay;
		DuelTuning.m_GrenadeFireDelay = DuelDefaults.m_GrenadeFireDelay;
		DuelTuning.m_LaserFireDelay = DuelDefaults.m_LaserFireDelay;
		DuelTuning.m_HammerHitFireDelay = DuelDefaults.m_HammerHitFireDelay;
		pParams = (int *)&DuelTuning;
	}
...
void CGameContext::OnTick()
{
	...
	TickFlightChess();
	TickGameInviteVotes();
	RefreshTurtleSoupDummyActivity();
	CheckUndercoverTimers();
	if(g_Config.m_SvMusicQQRelay && !m_QQPollInFlight &&
		Server()->Tick() >= m_LastQQPollTick + Server()->TickSpeed() * g_Config.m_SvMusicQQPollInterval)
	{
		PollQQMessages();
	}
	if(!m_ServerStateUpdateInFlight &&
		Server()->Tick() >= m_LastServerStateUpdateTick + Server()->TickSpeed() * g_Config.m_SvMusicStateUpdateInterval)
	{
	...
	if(m_VoteCloseTime)
	{
		...
		else
		{
			...
			if(m_VoteUpdate)
			{
				...
				for(int i = 0; i < MAX_CLIENTS; i++)
				{
					if(m_apPlayers[i] && !Server()->IsTurtleSoupDummy(i))
					{
				...
				for(int i = 0; i < MAX_CLIENTS; i++)
				{
					if(!m_apPlayers[i] || aVoteChecked[i] || Server()->IsTurtleSoupDummy(i))
						continue;
					...
					{
						if(!m_apPlayers[j] || aVoteChecked[j] || Server()->IsTurtleSoupDummy(j) || net_addr_comp_noport(apAddresses[j], apAddresses[i]) != 0)
							continue;
					...
					if(!IsKickVote() && !IsSpecVote() && g_Config.m_SvVoteVetoTime)
					{
						...
						for(int j = i; j < MAX_CLIENTS; j++)
						{
							...
							if(i != j && (!m_apPlayers[j] || Server()->IsTurtleSoupDummy(j) || net_addr_comp_noport(apAddresses[j], apAddresses[i]) != 0))
								continue;
...
void CGameContext::ProgressVoteOptions(int ClientId)
{
	...
	const int NumVoteOptions = GetMusicVoteOptionCount(ClientId) + m_NumVoteOptions;
	...
	char aaDescriptions[15][VOTE_DESC_LENGTH];

	while(CurIndex < NumVotesToSend)
	{
		if(!GetVoteOptionDescriptionForClient(ClientId, pPl->m_SendVoteIndex + CurIndex, aaDescriptions[CurIndex], sizeof(aaDescriptions[CurIndex])))
			break;

		switch(CurIndex)
		{
		case 0: OptionMsg.m_pDescription0 = aaDescriptions[CurIndex]; break;
		case 1: OptionMsg.m_pDescription1 = aaDescriptions[CurIndex]; break;
		case 2: OptionMsg.m_pDescription2 = aaDescriptions[CurIndex]; break;
		case 3: OptionMsg.m_pDescription3 = aaDescriptions[CurIndex]; break;
		case 4: OptionMsg.m_pDescription4 = aaDescriptions[CurIndex]; break;
		case 5: OptionMsg.m_pDescription5 = aaDescriptions[CurIndex]; break;
		case 6: OptionMsg.m_pDescription6 = aaDescriptions[CurIndex]; break;
		case 7: OptionMsg.m_pDescription7 = aaDescriptions[CurIndex]; break;
		case 8: OptionMsg.m_pDescription8 = aaDescriptions[CurIndex]; break;
		case 9: OptionMsg.m_pDescription9 = aaDescriptions[CurIndex]; break;
		case 10: OptionMsg.m_pDescription10 = aaDescriptions[CurIndex]; break;
		case 11: OptionMsg.m_pDescription11 = aaDescriptions[CurIndex]; break;
		case 12: OptionMsg.m_pDescription12 = aaDescriptions[CurIndex]; break;
		case 13: OptionMsg.m_pDescription13 = aaDescriptions[CurIndex]; break;
		case 14: OptionMsg.m_pDescription14 = aaDescriptions[CurIndex]; break;
		}
...
{
	const bool IsTurtleSoupDummy = Server()->IsTurtleSoupDummy(ClientId);
	if(IsTurtleSoupDummy)
	{
		m_TurtleSoupDummyClientID = ClientId;
		if(m_apPlayers[ClientId])
		{
			str_copy(m_apPlayers[ClientId]->m_TeeInfos.m_aSkinName, g_Config.m_SvTurtleSoupDummySkin, sizeof(m_apPlayers[ClientId]->m_TeeInfos.m_aSkinName));
			m_apPlayers[ClientId]->m_TeeInfos.m_UseCustomColor = 0;
			m_apPlayers[ClientId]->m_TeeInfos.m_ColorBody = 0;
			m_apPlayers[ClientId]->m_TeeInfos.m_ColorFeet = 0;
			m_apPlayers[ClientId]->SetInitialAfk(false);
			m_apPlayers[ClientId]->UpdatePlaytime();
			m_apPlayers[ClientId]->m_LastActionTick = Server()->Tick();
		}
	}
	if(m_TeeHistorianActive)
	{
	...
	if(!IsTurtleSoupDummy && !Server()->ClientPrevIngame(ClientId))
	{
...
void CGameContext::OnClientConnected(int ClientId, void *pData)
{
	...
	const int StartTeam = (!IsTurtleSoupDummy && (Spec || g_Config.m_SvTournamentMode)) ? TEAM_SPECTATORS : m_pController->GetAutoTeam(ClientId);
...
void CGameContext::OnClientDrop(int ClientId, const char *pReason)
{
	...
	AbortDuelFor(ClientId, "玩家断线");
	AbortGomokuFor(ClientId, "对方断线");
	AbortConnect4For(ClientId, "对方断线");
	AbortFlightChessFor(ClientId, "玩家断线");
	m_aGameInviteVotes[ClientId] = SGameInviteVote{};
	for(int OtherId = 0; OtherId < MAX_CLIENTS; OtherId++)
		if(HasGameInviteVote(OtherId) && m_aGameInviteVotes[OtherId].m_Inviter == ClientId)
			CloseGameInviteVote(OtherId);
	ClearChiDots(ClientId);
	ClearChidChatGrant(ClientId);
	m_RainbowName.Clear(ClientId);
	if(ClientId == m_TurtleSoupDummyClientID)
		m_TurtleSoupDummyClientID = -1;
...
void CGameContext::OnSayNetMessage(const CNetMsg_Cl_Say *pMsg, int ClientId, const CUnpacker *pUnpacker)
{
	...
	if(pMsg->m_pMessage[0] == '/')
	{
		...
		const char *pChidText = str_startswith_nocase(pCommand, "chid");
		if(pChidText && (*pChidText == '\0' || *pChidText == ' ' || *pChidText == '\t'))
		{
			if(!HasChidChatGrant(ClientId))
			{
				SendChatTarget(ClientId, "你没有 /chid 聊天框权限，或该临时授权已因改名/重连失效。");
				return;
			}
			while(*pChidText == ' ' || *pChidText == '\t')
				++pChidText;
			ExecuteChid(ClientId, pChidText);
			return;
		}
	...
	{
		if(HandleTurtleSoupChat(pMsg, ClientId))
			return;
		if(HandleUndercoverChat(pMsg, ClientId))
			return;
...
{
	if(str_comp_nocase(pMsg->m_pType, "option") == 0 && IsMusicSearchVoteOption(pMsg->m_pValue))
	{
		HandleMusicSearchVoteOption(ClientId, pMsg->m_pReason);
		return;
	}
	if(HasGameInviteVote(ClientId))
		return;
	...
	{
		if(IsMusicSkipVoteOption(pMsg->m_pValue))
		{
			HandleMusicSkipVoteOption(ClientId, aReason);
			return;
		}

		if(HandleMusicChooseVoteOption(ClientId, pMsg->m_pValue, aReason))
			return;

		if(IsMusicMenuVoteOption(ClientId, pMsg->m_pValue))
		{
			SendChatTarget(ClientId, "这是音乐菜单展示项");
			return;
		}
...
{
	if(HandleGameInviteVote(ClientId, pMsg->m_Vote))
	{
		if(m_apPlayers[ClientId])
			m_apPlayers[ClientId]->UpdatePlaytime();
		return;
...
bool CGameContext::HasGameInviteVote(int ClientId) const
{
	return ClientId >= 0 && ClientId < MAX_CLIENTS && m_aGameInviteVotes[ClientId].m_Type != GAME_INVITE_VOTE_NONE;
}

const char *GameInviteVoteReason(CGameContext::EGameInviteVoteType Type)
{
	switch(Type)
	{
	case CGameContext::GAME_INVITE_VOTE_DUEL: return "单挑";
	case CGameContext::GAME_INVITE_VOTE_GOMOKU: return "五子棋";
	case CGameContext::GAME_INVITE_VOTE_CONNECT4: return "四子棋";
	case CGameContext::GAME_INVITE_VOTE_FLIGHT_CHESS: return "飞行棋";
	default: return "游戏";
	}
}

const char *GameInviteVoteCommand(CGameContext::EGameInviteVoteType Type)
{
	switch(Type)
	{
	case CGameContext::GAME_INVITE_VOTE_DUEL: return "duel";
	case CGameContext::GAME_INVITE_VOTE_GOMOKU: return "gomoku";
	case CGameContext::GAME_INVITE_VOTE_CONNECT4: return "connect4";
	case CGameContext::GAME_INVITE_VOTE_FLIGHT_CHESS: return "fly";
	default: return "";
	}
}

bool CGameContext::ShowGameInviteVote(int ClientId, int Inviter, EGameInviteVoteType Type, int ExpireTick)
{
	if(ClientId < 0 || ClientId >= MAX_CLIENTS || Inviter < 0 || Inviter >= MAX_CLIENTS || !m_apPlayers[ClientId] || !m_apPlayers[Inviter] || Type == GAME_INVITE_VOTE_NONE || HasGameInviteVote(ClientId))
		return false;
	m_aGameInviteVotes[ClientId].m_Type = Type;
	m_aGameInviteVotes[ClientId].m_Inviter = Inviter;
	m_aGameInviteVotes[ClientId].m_ExpireTick = ExpireTick;
	SendGameInviteVote(ClientId);
	return true;
}

void CGameContext::SendGameInviteVote(int ClientId)
{
	if(!HasGameInviteVote(ClientId) || !m_apPlayers[ClientId])
		return;
	const SGameInviteVote &Invite = m_aGameInviteVotes[ClientId];
	if(Invite.m_Inviter < 0 || Invite.m_Inviter >= MAX_CLIENTS || !m_apPlayers[Invite.m_Inviter])
		return;

	char aDescription[VOTE_DESC_LENGTH];
	str_format(aDescription, sizeof(aDescription), "%s 的邀请", Server()->ClientName(Invite.m_Inviter));
	const int Timeout = maximum((Invite.m_ExpireTick - Server()->Tick() + Server()->TickSpeed() - 1) / Server()->TickSpeed(), 1);
	::CNetMsg_Sv_VoteSet Msg6;
	Msg6.m_Timeout = Timeout;
	Msg6.m_pDescription = aDescription;
	Msg6.m_pReason = GameInviteVoteReason(Invite.m_Type);
	protocol7::CNetMsg_Sv_VoteSet Msg7;
	Msg7.m_Timeout = Timeout;
	Msg7.m_pDescription = aDescription;
	Msg7.m_pReason = GameInviteVoteReason(Invite.m_Type);
	Msg7.m_ClientId = Invite.m_Inviter;
	Msg7.m_Type = protocol7::VOTE_START_OP;
	if(Server()->IsSixup(ClientId))
		Server()->SendPackMsg(&Msg7, MSGFLAG_VITAL, ClientId);
	else
	...
	CNetMsg_Sv_YourVote YourVote = {0};
	Server()->SendPackMsg(&YourVote, MSGFLAG_VITAL, ClientId);
	CNetMsg_Sv_VoteStatus Status = {0};
	Status.m_Total = 1;
	Status.m_Pass = 1;
	Server()->SendPackMsg(&Status, MSGFLAG_VITAL, ClientId);
}

void CGameContext::CloseGameInviteVote(int ClientId)
{
	if(!HasGameInviteVote(ClientId))
		return;
	m_aGameInviteVotes[ClientId] = SGameInviteVote{};
	if(!m_apPlayers[ClientId])
		return;
	SendVoteSet(ClientId);
	CNetMsg_Sv_YourVote YourVote = {m_VoteCloseTime ? m_apPlayers[ClientId]->m_Vote : 0};
	Server()->SendPackMsg(&YourVote, MSGFLAG_VITAL, ClientId);
	if(m_VoteCloseTime)
		m_VoteUpdate = true;
}

bool CGameContext::IsGameInviteStillPending(int ClientId, const SGameInviteVote &Invite) const
{
	if(ClientId < 0 || ClientId >= MAX_CLIENTS || Invite.m_Inviter < 0 || Invite.m_Inviter >= MAX_CLIENTS || !m_apPlayers[ClientId] || !m_apPlayers[Invite.m_Inviter])
		return false;
	switch(Invite.m_Type)
	{
	case GAME_INVITE_VOTE_DUEL: return m_aDuelRequester[ClientId] == Invite.m_Inviter;
	case GAME_INVITE_VOTE_GOMOKU: return m_aGomokuRequester[ClientId] == Invite.m_Inviter;
	case GAME_INVITE_VOTE_CONNECT4: return m_aConnect4Requester[ClientId] == Invite.m_Inviter;
	case GAME_INVITE_VOTE_FLIGHT_CHESS:
		for(const SFlightChessInvite &FlightInvite : m_aFlightChessInvites)
			if(FlightInvite.m_Active && FlightInvite.m_Inviter == Invite.m_Inviter)
				for(int Slot = 1; Slot < 4; Slot++)
					if(FlightInvite.m_aPlayers[Slot] == ClientId)
						return !FlightInvite.m_aAccepted[Slot];
		return false;
	default: return false;
	}
}

void CGameContext::TickGameInviteVotes()
{
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ClientId++)
		if(HasGameInviteVote(ClientId) && (Server()->Tick() >= m_aGameInviteVotes[ClientId].m_ExpireTick || !IsGameInviteStillPending(ClientId, m_aGameInviteVotes[ClientId])))
			CloseGameInviteVote(ClientId);
}

bool CGameContext::HandleGameInviteVote(int ClientId, int Vote)
{
	if(!HasGameInviteVote(ClientId))
		return false;
	if(Vote == 0)
		return true;
	const EGameInviteVoteType Type = m_aGameInviteVotes[ClientId].m_Type;
	const char *pCommand = GameInviteVoteCommand(Type);
	CloseGameInviteVote(ClientId);
	if(!pCommand[0])
		return true;

	char aCommand[64];
	str_format(aCommand, sizeof(aCommand), "%s %s", pCommand, Vote > 0 ? "accept" : "decline");
	Console()->SetFlagMask(CFGFLAG_CHAT);
	Console()->SetAccessLevel(IConsole::ACCESS_LEVEL_USER);
...
static void UnescapeAlertNewlines(char *pBuf)
{
	int Read;
	int Write;
	for(Read = 0, Write = 0; pBuf[Read]; Read++, Write++)
	{
		if(pBuf[Read] == '\\' && pBuf[Read + 1] == 'n')
		{
			pBuf[Write] = '\n';
			Read++;
		}
		else if(Read != Write)
			pBuf[Write] = pBuf[Read];
	}
	pBuf[Write] = '\0';
}
...
void CGameContext::ConModAlert(IConsole::IResult *pResult, void *pUserData)
{
	...
	UnescapeAlertNewlines(aBuf);
	pSelf->SendModeratorAlert(Victim, aBuf);
}

void CGameContext::ConModAlertAll(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	char aBuf[1024];
	...
	for(int ClientId = 0; ClientId < pSelf->Server()->MaxClients(); ClientId++)
		if(pSelf->m_apPlayers[ClientId])
			pSelf->SendModeratorAlert(ClientId, aBuf);
}
...
void CGameContext::ConColor(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(ClientId < 0 || ClientId >= MAX_CLIENTS || !pSelf->m_apPlayers[ClientId] || !pSelf->Server()->ClientIngame(ClientId))
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "color", "invalid or inactive client id");
		return;
	}
	if(g_Config.m_SvTeam == SV_TEAM_FORCED_SOLO)
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "color", "color IDs require DDRace team state; sv_team is forced solo");
		return;
	}
	if(!pSelf->m_RainbowName.IsEnabled(ClientId) && pSelf->GetDDRaceTeam(ClientId) != TEAM_FLOCK)
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "color", "color IDs can only be enabled for a player in DDRace team 0");
		return;
	}

	const bool Enabled = pSelf->m_RainbowName.Toggle(ClientId);

	char aBuf[256];
	str_format(aBuf, sizeof(aBuf), "display-only rainbow ID %s for %d:%s (real DDRace team remains %d)",
		Enabled ? "enabled" : "disabled", ClientId, pSelf->Server()->ClientName(ClientId), pSelf->GetDDRaceTeam(ClientId));
	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "color", aBuf);
	pSelf->SendChatTarget(ClientId, Enabled ? "炫彩 ID 已开启（仅展示队伍会变化，真实队伍仍为 0）" : "炫彩 ID 已关闭");
}

void CGameContext::ClearChidChatGrant(int ClientId)
{
	if(ClientId < 0 || ClientId >= MAX_CLIENTS)
		return;
	m_aChidChatGrants[ClientId] = {};
}

bool CGameContext::HasChidChatGrant(int ClientId)
{
	if(ClientId < 0 || ClientId >= MAX_CLIENTS)
		return false;

	SChidChatGrant &Grant = m_aChidChatGrants[ClientId];
	if(Grant.m_UniqueClientId == 0)
		return false;
	if(!m_apPlayers[ClientId] || !Server()->ClientIngame(ClientId) ||
		Grant.m_UniqueClientId != m_apPlayers[ClientId]->GetUniqueCid() ||
		str_comp(Grant.m_aName, Server()->ClientName(ClientId)) != 0)
	{
		ClearChidChatGrant(ClientId);
		return false;
	}
...
void CGameContext::ConChidGrant(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = static_cast<CGameContext *>(pUserData);
	const int ClientId = pResult->GetInteger(0);
	const char *pExactName = pResult->GetString(1);
	if(ClientId < 0 || ClientId >= MAX_CLIENTS || !pSelf->m_apPlayers[ClientId] || !pSelf->Server()->ClientIngame(ClientId))
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "chid", "grant failed: client ID is not an active player");
		return;
	}
	if(!pExactName[0] || str_comp(pExactName, pSelf->Server()->ClientName(ClientId)) != 0)
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "chid", "grant failed: exact current nickname is required (quote names containing spaces)");
		return;
	}

	SChidChatGrant &Grant = pSelf->m_aChidChatGrants[ClientId];
	Grant.m_UniqueClientId = pSelf->m_apPlayers[ClientId]->GetUniqueCid();
	str_copy(Grant.m_aName, pExactName, sizeof(Grant.m_aName));

	char aBuf[256];
	str_format(aBuf, sizeof(aBuf), "chat /chid granted to %d:%s for this connection only", ClientId, pExactName);
	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "chid", aBuf);
	pSelf->SendChatTarget(ClientId, "管理员已授予你聊天框 /chid 权限；改名、断线或重连后自动失效。");
}

void CGameContext::ConChidRevoke(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = static_cast<CGameContext *>(pUserData);
	const int ClientId = pResult->GetInteger(0);
	const char *pExactName = pResult->GetString(1);
	if(ClientId < 0 || ClientId >= MAX_CLIENTS || !pSelf->m_apPlayers[ClientId] || !pSelf->Server()->ClientIngame(ClientId) ||
		!pExactName[0] || str_comp(pExactName, pSelf->Server()->ClientName(ClientId)) != 0 || !pSelf->HasChidChatGrant(ClientId))
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "chid", "revoke failed: ID, exact current nickname, and active grant must all match");
		return;
	}

	pSelf->ClearChidChatGrant(ClientId);
	char aBuf[256];
	str_format(aBuf, sizeof(aBuf), "chat /chid revoked from %d:%s", ClientId, pExactName);
	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "chid", aBuf);
	pSelf->SendChatTarget(ClientId, "你的聊天框 /chid 权限已被管理员撤销。");
}
...
void CGameContext::OnConsoleInit()
{
	...
	Console()->Register("download_song", "i[vote_id]", CFGFLAG_SERVER, ConDownloadSong, this, "Add a voted song to the queue");
	Console()->Register("addmusic", "r[song_name]", CFGFLAG_SERVER, ConSong, this, "Play a song from /data/musicso directory");
	Console()->Register("queue_search", "r[song-name]", CFGFLAG_SERVER, ConQueueSearch, this, "Search songs for queue_insert_after in the remote console");
	Console()->Register("queue_insert_after", "i[after-queue-number] i[search-result-number]", CFGFLAG_SERVER, ConQueueInsertAfter, this, "Insert an admin-selected search result after a queue entry");
	Console()->Register("queue_list", "", CFGFLAG_SERVER, ConQueueList, this, "List the song queue with the numbers used by queue_insert_after");
	...
	Console()->Register("mod_alert", "v[id] r[message]", CFGFLAG_SERVER, ConModAlert, this, "Send a moderator alert message to one player");
	Console()->Register("mod_alert_all", "r[message]", CFGFLAG_SERVER, ConModAlertAll, this, "Send a moderator alert message to all players");
	...
	Console()->Register("color", "i[client-id]", CFGFLAG_SERVER, ConColor, this, "Toggle a display-only rainbow ID for a DDRace team 0 player");
	Console()->Register("chid_grant", "i[client-id] r[exact-name]", CFGFLAG_SERVER, ConChidGrant, this, "Grant chat /chid to this exact current client ID and name until rename or disconnect");
...
void CGameContext::RegisterChatCommands()
{
	...
	Console()->Register("myrank", "", CFGFLAG_CHAT | CFGFLAG_SERVER, ConChatMyRank, this, "查看我的点歌记录");
	Console()->Register("bindqq", "", CFGFLAG_CHAT | CFGFLAG_SERVER, ConChatBindQQ, this, "生成 QQ 身份绑定验证码");
	Console()->Register("bindstatus", "", CFGFLAG_CHAT | CFGFLAG_SERVER, ConChatBindStatus, this, "查看 QQ 身份绑定状态");
	Console()->Register("jrrp", "", CFGFLAG_CHAT | CFGFLAG_SERVER, ConChatJrrp, this, "查看今日人品");
	Console()->Register("roll", "?r[dice]", CFGFLAG_CHAT | CFGFLAG_SERVER, ConChatRoll, this, "掷骰子，例如 /roll 100 或 /roll 2d6");
	Console()->Register("pick", "r[options]", CFGFLAG_CHAT | CFGFLAG_SERVER, ConChatPick, this, "从选项中选择一个，例如 /pick A | B | C");
	Console()->Register("guess", "?r[answer]", CFGFLAG_CHAT | CFGFLAG_SERVER, ConChatGuess, this, "猜当前正在播放的歌曲");
	Console()->Register("guessrank", "", CFGFLAG_CHAT | CFGFLAG_SERVER, ConChatGuessRank, this, "查看猜歌排行榜");
	Console()->Register("uc", "?r[command]", CFGFLAG_CHAT | CFGFLAG_SERVER, ConChatUndercover, this, "谁是卧底小游戏");
	Console()->Register("duel", "?s[player-name|accept|decline|cancel] ?r[weapons]", CFGFLAG_CHAT, ConChatDuel, this, "发起或处理原地单挑");
	Console()->Register("gomoku", "?s[player-name|accept|decline|cancel]", CFGFLAG_CHAT, ConChatGomoku, this, "发起或处理五子棋");
	Console()->Register("connect4", "?s[player-name|accept|decline|cancel]", CFGFLAG_CHAT, ConChatConnect4, this, "发起或处理四子棋");
	Console()->Register("fly", "?r[player-name...|accept|decline|cancel]", CFGFLAG_CHAT, ConChatFlightChess, this, "邀请至多三名玩家玩飞行棋");
	Console()->Register("chid", "?r[text]", CFGFLAG_SERVER, ConChid, this, "动态生成汉字字幕，例如 chid 0.7 你好");
